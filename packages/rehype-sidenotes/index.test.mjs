    ...
    element(
      ...
      items.map(({ id, content }) =>
        element("li", { id: `user-content-fn-${id}` }, [
          element("p", {}, [
            ...content,
            element("a", { href: `#user-content-fnref-${id}`, dataFootnoteBackref: "" }, [
              text("↩"),
            ]),
          ]),
        ]),
...
describe("rehypeSidenotes", () => {
  it("transforms a footnote ref into a sidenote", () => {
    const tree = run(
      root(
        element("p", {}, [text("Some text"), footnoteRef("1", 1), text(" more text.")]),
    ...
    assert.deepEqual(sidenote.children[0].properties.className, ["sidenoteNumber"]);
  ...
  it("handles multiple footnotes", () => {
    ...
    const sidenotes = para.children.filter(
      (c) => c.tagName === "span" && c.properties?.className?.includes("sidenote"),
    );
  ...
  it("no-op when no footnotes section", () => {
    const tree = root(element("p", {}, [text("Some text"), footnoteRef("1", 1)]));
  ...
  it("preserves the footnotes section for mobile fallback", () => {
    ...
    const section = tree.children.find(
      (c) => c.tagName === "section" && c.properties?.dataFootnotes !== undefined,
    );
