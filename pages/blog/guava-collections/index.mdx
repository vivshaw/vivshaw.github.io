...
import metaImage from "./josefin-HtCgLIDy-2U-unsplash.jpg";

export const meta = {
  title: "Beautify Your Java Collections with Google Guava",
  blurb: "",
  date: new Date("2017-02-02"),
  image: { alt: "green and white leaf plant", src: metaImage },
  next: ["linux-on-windows"],
  slug: "guava-collections",
  tags: ["java", "guava"],
};
...
frequencies.entrySet()
           .forEach(entry ->
               System.out.format("There are %d %ss.%n", entry.getCount(), entry.getElement())
           );
```
