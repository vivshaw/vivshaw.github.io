...
- creating Tufte CSS style sidenotes is a little tricky. my first attempt, which used the ol' Tufte CSS float trick, _almost_ worked, except the sidenotes' height counted as part of the paragraph height. this meant any long sidenote would push the content way down the page. it turns out this is because i used `display: grid` for the blog layout, grid children establish a [block formatting context](https://developer.mozilla.org/en-US/docs/Web/CSS/Guides/Display/Block_formatting_context), and this causes everything inside to be calculated into the height.
