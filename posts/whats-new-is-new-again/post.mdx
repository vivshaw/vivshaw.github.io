...
This was not to be. I continued to have a bone to pick with my website. Particularly, I was unsatisfied with these two things: the legibility of my posts, and the loss of visual identity. My technical choices made these annoying to resolve. So, 2026 brings a new version, significantly changed in both design and architecture from before. Rejoice— you may now listen to me navel-gaze for 2500 words about the choices I made along the way.
...
Dialing this in took a long time, and involved some _truly awkward_ in-between states. But constraint-based techniques get there in the end! By steadily ruling out more and more inconsistencies and horrible choices, I steadily approached a system that was useful to build with and pleasant to look at. I think the next step is to fill out my components- I only have 4 in the main system (`<Heading>`, `<Text>`, `<Link>`, `<Pill>`). I don't wanna go wild with it, but I could see a role for a few more- perhaps a `<Surface>`, or some layout components.

## On progressive enhancement

Oddly, in the past progressive enhancement was something I've both cared about and given very little thought to. I'd "accomplish" it primarily by avoiding JS features entirely. But really, I'd rather have a site that can leverage those features when they are available, and gracefully degrade when they are not.

This site now does that. I set a `no-js` class on the `html` element, and then added a script to swap it out for a `js` class when JS is available. The majority of my components are statically rendered. Those components that are dynanmic watch this `js` class, and only if it's present do they do their fancy stuff. The fallback behavior differs from component to component, but generally it's either to not render at all, or render a static fallback UI[^fallbacks].

[^fallbacks]: For example, the showy/hidey sticky navbar becomes a stationary navbar at the top of the page.

I made sure that my light and dark theme are fully functional without JS. I default to the system's `prefers-color-scheme` setting, and only set a specific theme if JS is active. I also made sure to use semantic elements whenever possible. So, even without JS _and CSS_, it should at least be usable.
...
Vanilla Extract _did_ accomplish these things. As I adopted it, though, I discovered progressively more challenges. One challenge was the number of different ways of doing things with different drawbacks. Once I had my `<Box>` up and running, there were four ways to style: raw Vanilla Extract stylesheets, [Recipes](vanilla-extract.style/documentation/packages/recipes/) to define components with variants, [Sprinkles](https://vanilla-extract.style/documentation/packages/sprinkles/) in the stylesheet, and Sprinkles as component props. I'd _hoped_ to do everything with Sprinkles, and ignore these lower layers. First, I found that my light theme and dark theme utils, written to work with raw Vanilla Extract stylesheets, didn't work with them. This pushed some of my light/dark aware components back into the stylesheets. Then I discovered that handling pseudo-selectors like `:focus` and `:hover` causes [a combinatorial explosion of styles](https://github.com/vanilla-extract-css/vanilla-extract/discussions/1478), making it preferable to keep 'em in the stylesheets. Then I discovered [Sprinkles shorthands](https://vanilla-extract.style/documentation/packages/sprinkles/#shorthands), which were a nice way to put some commonly grouped styles together into a single thing... _but_, only in the Sprinkles layer. The consequence of this was that my styles were a mess of different techniques, all with different tradeoffs, and no clear path toward unity.
