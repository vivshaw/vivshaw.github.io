...
/**
 * gets the social preview image for the given page.
 * each page has its own image generated by a sibling `og.png` route handler.
 */
function getPageOgImage(data: SeoData, pageName: string) {
  const url = (() => {
    switch (data.type) {
      case "home":
        return "/og.png";
      case "topLevel":
        return `/${data.slug}/og.png`;
      case "post":
        return `/blog/${data.slug}/og.png`;
    }
  })();
  return { url, width: 1200, height: 630, alt: pageName };
}
...
export function metadataHelper(data: SeoData): Metadata {
  ...
  const pageOgImage = getPageOgImage(data, pageName);
